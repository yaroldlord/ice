...
$jscomp.polyfill('Map', function (b) {
    var g = !$jscomp.ASSUME_NO_NATIVE_MAP && function () {
        ...
        try {
            ...
            if (c.get(e) != 's' || c.size != 1 || c.get({x: 4}) || c.set({x: 4}, 't') != c || c.size != 2) {
                return !1
            }
            var d = c.entries();
            var a = d.next();
    ...
    a.prototype.set = function (e, d) {
        ...
        if (!a.entry) {
            a.entry = {next: this.head_, previous: this.head_.previous, head: this.head_, key: e, value: d};
            a.list.push(a.entry);
...
$jscomp.polyfill('Set', function (b) {
    var c = !$jscomp.ASSUME_NO_NATIVE_SET && function () {
        if (!b || !b.prototype.entries || typeof Object.seal != 'function') {
            return !1
        }
        try {
            ...
            if (!c.has(d) || c.size != 1 || c.add(d) != c || c.size != 1 || c.add({x: 4}) != c || c.size != 2) {
                return !1
            }
            var e = c.entries();
            var a = e.next();
...
$jscomp.polyfill('WeakSet', function (b) {
    function isConformant() {
        if (!b || !Object.seal) {
            return !1
        }
...
tinyMCE.init({
    ...
    theme_advanced_buttons2: 'search,replace,separator,sub,sup, charmap,separator , unlink, removeformat , separator, visualaid, table, separator, cleanup ,code,ice_smartquotes',
    theme_advanced_buttons3: 'bullist,numlist,row_props,cell_props,|,row_before,row_after,delete_row,|,col_before,col_after,delete_col,|,ice_togglechanges,ice_toggleshowchanges,iceacceptall,icerejectall,iceaccept,icereject,',
    valid_elements: 'a[href],b/strong,i/em,u,ul,li,ol,br,sub,sup,table[style=border-collapse:collapse|border=1|cellspacing|cellpadding|width|height|align],tr[style|width|height|align|valign|rowspan|colspan],td[style|width|height|align|valign|rowspan|colspan],div[align],p[align<center?justify?left?right|style|title],',
    inline_styles: !1,
    paste_auto_cleanup_on_paste: !0,
    paste_text_use_dialog: !0,
    paste_retain_style_properties: 'margin, border-width, border-style, bgcolor, background, bordercolor, ul, li, ol, page-break-before, page-break-after, padding, width, height, font-size, font-weight, font-family, text-align, text-decoration, float, display, line-height, align',
    paste_convert_headers_to_strong: !0,
    ice: {isForceTracking: !1, isTracking: !1, user: {name: 'Utilisateur inconnu', id: 1}}
});
