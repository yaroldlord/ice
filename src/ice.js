    ...
    InlineChangeEditor.prototype = {
        ...
        findTrackTags: function () {
            ...
            ice.dom.each(ice.dom.find(this.element, '.' + changeTypeClasses.join(', .')), function (i, el) {
                ...
                for (var idx = 0; idx < classList.length; idx++) {
                    var styleReg = new RegExp(self.stylePrefix + '-(\\d+)').exec(classList[idx]);
                    if (styleReg) styleIndex = styleReg[1];
                    var ctnReg = new RegExp('(' + changeTypeClasses.join('|') + ')').exec(classList[idx]);
        ...
        handleEvent: function (e) {
            ...
            if (e.type === 'mouseup') {
                ...
                var needsToBubble1 = this.keyPress(e);
                if (!needsToBubble1) e.preventDefault();
                return needsToBubble1;
            } else if (e.type === 'keydown') {
                var needsToBubble2 = this.keyDown(e);
                if (!needsToBubble2) e.preventDefault();
                return needsToBubble2;
...
//            range.setStart(range.endContainer.parentNode.nextSibling, 0);
        ...
        _getNoTrackElement: function (node) {
            ...
            return ice.dom.is(node, noTrackSelector) ? node : (ice.dom.parents(node, noTrackSelector)[0] || null);
        },
...
//      range.collapse(false);
        ...
        _addNodeTracking: function (contentNode, range, moveLeft) {
            ...
            if (contentAddNode && this._currentUserIceNode(contentAddNode)) {
                ...
                if ((ice.dom.hasNoTextOrStubContent(cleanNode[0]))) {
        ...
        _handleAncillaryKey: function (e) {
            ...
            switch (key) {
                ...
                case 32:
                    preventDefault = true;
                    var range1 = this.getCurrentRange();
                    this._moveRangeToValidTrackingPos(range1, range1.startContainer);
                    this.insert('\u00A0', range1);
                    break;
                default:
                    // Ignore key.
